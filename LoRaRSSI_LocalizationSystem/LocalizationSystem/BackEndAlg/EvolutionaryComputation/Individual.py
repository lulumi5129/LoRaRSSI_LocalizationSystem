#
# Individual.py
#
#

import math
from LocalizationSystem.Node.node import Node

#Base class for all individual types
#
class Individual(Node):
    """
    Individual
    """
    minMutRate=1e-100
    maxMutRate=1
    learningRate=None
    uniprng=None
    normprng=None
    fitFunc=None

    def __init__(self, x=None, y=None, z=None, _dimensions=None, _ID=None):
        super().__init__(x, y, z, dimensions=_dimensions, headerName='EV3', ID=_ID)
        self.fit=self.__class__.fitFunc(self)
        self.mutRate=self.uniprng.uniform(0.9,0.1) #use "normalized" sigma
            
    def mutateMutRate(self):
        self.mutRate=self.mutRate*math.exp(self.learningRate*self.normprng.normalvariate(0,1))
        if self.mutRate < self.minMutRate: self.mutRate=self.minMutRate
        if self.mutRate > self.maxMutRate: self.mutRate=self.maxMutRate
            
    def evaluateFitness(self):
        if self.fit == None: self.fit=self.__class__.fitFunc(self)
        
    def crossover(self, other):
        #perform crossover "in-place"
        for i in range(len(self.coordinate)) :
            tmp = (self.coordinate[i] + other.coordinate[i])/2 
            
            self.coordinate[i] += tmp
            other.coordinate[i] -= tmp
            
        self.setXYZ()
        other.setXYZ()
        self.fit=None
        other.fit=None
        
    def mutate(self):
        self.mutateMutRate() #update mutation rate
        
        for i in range(len(self)) :
            self.coordinate[i] = self.coordinate[i] * self.mutRate
        
        self.setXYZ()
        self.fit=None
        
    def __str__(self):
        return super().__str__() + '\t'+'%0.8e'%self.fit+'\t'+'%0.8e'%self.mutRate
