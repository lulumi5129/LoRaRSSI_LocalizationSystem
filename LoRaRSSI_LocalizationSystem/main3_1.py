...
def main(argv=None):
    ...
    LocalizationConfigFilePath = './config/main2/Localization_2D.cfg'
    if argv is None:
        argv = sys.argv
    ...
    try:
        ...
        for target in LS_Obj.targetList :
            ...
            proc = multiprocessing.Process(name=str(target.nodeName) + 'EV3_1', target=EV3_1.ev3_multiprocess, args=(q1, settingPath1, LS_Obj, target,))
            jobs.append(proc)
            proc.start()
            proc = multiprocessing.Process(name=str(target.nodeName) + '_EV3_2', target=EV3_2.ev3_multiprocess, args=(q2, settingPath2, LS_Obj, target,))
            jobs.append(proc)
            proc.start()
        ...
        #wait run end
        for proc in jobs :
            if proc.is_alive() : proc.join(100)
            
        print('b')
        #get result
        EV3_1_res = {}
        EV3_2_res = {}
        for i in range(int(len(jobs)/2)) :
            print(i)
            res_1 = q1.get()
            res_2 = q2.get()
            EV3_1_res[res_1[0].nodeName] = res_1[1]
            EV3_2_res[res_2[0].nodeName] = res_2[1]
            print(str(res_1[0].nodeName) + '\t' + str(res_1[1]))
            print(str(res_2[0].nodeName) + '\t' + str(res_2[1]))
